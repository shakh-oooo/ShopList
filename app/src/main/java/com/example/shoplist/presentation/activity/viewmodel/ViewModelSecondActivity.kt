package com.example.shoplist.presentation.activity.viewmodel
...
import com.example.shoplist.domain.usecases.items.AddShopItemUSeCase
...
class ViewModelSecondActivity : ViewModel() {
    ...
    fun editItemsXZ(name: String?, count: String?) {
        val name = parsingName(name)
        val count = parsingCount(count)
        val validat = validateInput(name, count)
        if (validat) {
